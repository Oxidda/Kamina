...
namespace Kamina.Logic.Core.Games
{
    public class HangmanLogic : IHangmanLogic
    {
        ...
        {
            HangmanGame game = await _state.GetGameAsync(context.Guild.Id);

            if (game == null)
            {
                game = new HangmanGame();
                string word = _state.GetRandomWord();
                if (!String.IsNullOrEmpty(word))
                {
                    game.TargetWord = word.ToLower();
                    GuildId = context.Guild.Id;

                    await _state.AddGameAsync(GuildId, game);
                    await SendResponse(context, game, String.Empty);
                }
                Logger.Log($"Starting game {game.TargetWord}");
            }
        }
        ...
        private async Task Client_MessageReceived(SocketMessage arg)
        {
            ...
            if (await context.IsTextGames())
            {
                await Task.Run(async () =>
                {
                    ...
                    {
                        var game = await _state.GetGameAsync(context.Guild.Id);
                        if (game != null)
                        {
        ...
        {
            await _state.RemoveGameAsync(context.Guild.Id);
        }

        private async Task SendResponse(ICommandContext context, HangmanGame game, string answer)
        {
            ...
            string hangman = await GetHangman(game);
